...
export function WorksSection() {
  ...
  // Curatorial layout: featured items get larger grid spans
  const getGridClass = (index: number, featured?: boolean) => {
    if (featured && index === 0) return 'md:col-span-8 md:row-span-2';
    if (featured && index === 2) return 'md:col-span-4 md:row-span-2';
      ...
      <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16">
        ...
        >
          {items.map((item, index) => (
            <div
              key={item.slug}
              className={`work-card ${getGridClass(index, item.featured)}`}
            >
              <TiltCard tiltAmount={6} scale={1.01}>
                <Link
                  to={`/collection/${item.slug}`}
                  className="group relative block overflow-hidden bg-ink-light border border-border-subtle"
                  data-cursor="view"
                >
                  {/* Image */}
                  <div
